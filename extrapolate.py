...
def get_alessio_cbs(hf_X, corr_X, hf_Y, corr_Y, X=2, Y=3, convert_Hartree=False,shift=0.0,output=True):
    
    if Y != X+1:
        print('Y does not equal X+1')

    alpha = (hf_Y - hf_X)/(np.exp(-1.5*Y) - np.exp(-1.5*(X)))
    beta = (corr_Y - corr_X)/(Y**(-3) - X**(-3))

    hf_cbs = hf_Y - np.exp(-1.5*Y)*alpha
    corr_cbs = corr_Y - beta*(Y**(-3))
    if convert_Hartree == True:
        if output==True:
            print('CBS({0}/{1}) HF: {2:.9f} Corr: {3:.9f} Tot: {4:.9f}'.format(X,Y, hf_cbs*Hartree + shift, corr_cbs*Hartree , (hf_cbs+corr_cbs)*Hartree + shift))
        return hf_cbs*Hartree + shift, corr_cbs*Hartree, (hf_cbs+corr_cbs)*Hartree
